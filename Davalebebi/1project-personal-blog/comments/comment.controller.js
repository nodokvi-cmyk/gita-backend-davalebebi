...
commentRouter.get("/:commentId", isValidMongoIdMiddleware("commentId"), async (req, res) => {
...
commentRouter.delete("/:commentId", isValidMongoIdMiddleware("commentId"), isAuthMiddleware, resourcePermissionMiddleware(commentModel, "commentId"), async(req, res) => {
    const deletedComment = await commentService.deleteComment(req.params.commentId)
    res.json({deleted: true, data: deletedComment})
})

commentRouter.put("/:commentId", isValidMongoIdMiddleware("commentId"), isAuthMiddleware, resourcePermissionMiddleware(commentModel, "commentId"), validateMiddleware(commentDto), async (req, res) => {
